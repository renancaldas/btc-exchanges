...
router.route('/last/:exchange')
  .get(function(req, res, next) {  
  	...
  	switch(req.params.exchange) {
  		...
  		case 'mercadobitcoin':
  			getLastPrice('https://www.mercadobitcoin.net/api/ticker').then(function(result){
  				Responses(res).success(JSON.parse(result).ticker.last);
  			}, function(err){
  				Responses(res).serverError(err);
  			});
  			break;
  		...
  		case 'foxbit':
  			getLastPrice('https://api.blinktrade.com/api/v1/BRL/ticker').then(function(result){
  				Responses(res).success(JSON.parse(result).last);
  			}, function(err){
  				Responses(res).serverError(err);
  			});
  			break;
      ...
      case 'satoshitango':
        getLastPrice('https://www.satoshitango.com/api/ticker?currency=ARS').then(function(result){
          Responses(res).success(JSON.parse(result).bid);
        }, function(err){
          Responses(res).serverError(err);
        });
        break;
